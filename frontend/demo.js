...
function adaptBackendProduct(product){
  const category=/кабел/i.test(product.category)?'cable':/выключател/i.test(product.category)?'breaker':/светиль/i.test(product.category)?'light':'other';
  return {...product,category,categoryName:product.category,warehouses:{[product.warehouse||'Демо-склад']:product.stock||0},tags:product.tags||[]};
}
...
function selectChat(id){if(!state.chats.some(chat=>chat.id===id))return;expirePending();state.chatId=id;state.lastProduct=null;const last=[...currentChat().messages].reverse().find(message=>message.kind==='query');if(last)state.lastProduct=findProduct(last.text);persist();renderConversation();renderHistory();toggleHistory(false);refreshServerCart();}
function newChat(){expirePending();const chat={id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,title:'Новый чат',messages:[]};state.chats.unshift(chat);state.chats=state.chats.slice(0,15);state.chatId=chat.id;state.lastProduct=null;state.cart=[];persist();renderConversation();renderHistory();renderCart();toggleHistory(false);}
...
function renderCatalog(list=products){const root=$('#product-grid');root.replaceChildren();$('#results-label').textContent=`Демо каталог · ${list.length} позиций`;if(!list.length){root.append(element('p','',copy[state.lang].noMatch));return;}list.forEach(product=>{const card=element('article','product-card');const visual=window.QuantMedia?.createThumbnail(product)||element('div','product-card__visual');if(!window.QuantMedia?.get(product.sku))visual.append(svg(product.category==='breaker'?'compare':product.category==='cable'?'box':'spark'));const category=element('span','product-card__type',product.categoryName);const title=element('h3','',product.name);const warehouses=Object.entries(product.warehouses).map(([name,count])=>`${name}: ${count} шт.`).join(' · ');const meta=element('div','product-card__meta',`Арт. ${product.sku} · ${warehouses}`);const pill=element('span','stock-pill',stock(product)>0?`${copy[state.lang].stock}: ${stock(product)} шт.`:copy[state.lang].out);const footer=element('div','product-card__footer');footer.append(element('strong','',money(product.price)));const ask=element('button','',copy[state.lang].ask);ask.type='button';ask.addEventListener('click',()=>send(`${state.lang==='kz'?'Тауарды көрсету':'Покажи товар'}: ${product.sku}`));footer.append(ask);card.append(visual,category,title,meta,pill,footer);root.append(card);});}
...
function productCard(product,parent){
  const card=element('div','chat-product');const photo=window.QuantMedia?.createThumbnail(product,'chat');if(photo)card.append(photo);const head=element('div','chat-product__head');head.append(element('strong','',product.name));
...
function renderCart(){const root=$('#cart-items');root.replaceChildren();if(!state.cart.length)root.textContent=copy[state.lang].cartEmpty;else state.cart.forEach(line=>{const row=element('div','cart-line');row.append(element('span','',`${line.name} × ${line.quantity}`),element('strong','',money(line.price*line.quantity)));root.append(row);});const count=state.cart.reduce((sum,line)=>sum+line.quantity,0);const total=state.cart.reduce((sum,line)=>sum+line.price*line.quantity,0);$('#cart-count').textContent=count;$('#cart-total').textContent=money(total);$('#checkout-button').disabled=count===0;}
...
function showBackendProducts(items=[]){const bubble=$('#chat-messages .message.assistant:last-child');if(!bubble)return;items.map(adaptBackendProduct).forEach(product=>productCard(product,bubble));scrollChat();}
async function refreshServerCart(){try{const response=await fetch(`/api/cart/${encodeURIComponent(state.chatId)}`);if(!response.ok)return;const data=await response.json();state.cart=data.items||[];renderCart();}catch{}}
async function applyBackendResponse(data){const bubble=addText(data.message||'Не удалось получить ответ.');showBackendProducts(data.products||[]);if(data.requires_confirmation){const actions=element('div','chat-product__actions');addButton(actions,'Да, добавить',()=>send('да, добавь')).classList.add('chat-product__primary');addButton(actions,'Отмена',()=>send('отмена'));bubble.append(actions);}if(data.action?.type==='confirm_add'){const response=await fetch('/api/cart/confirm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_id:state.chatId,sku:data.action.sku,quantity:data.action.quantity})});const cart=await response.json();if(!response.ok){addText(cart.detail||'Не удалось добавить товар в корзину.');return;}addText(cart.message);state.cart=cart.items||[];renderCart();}}
async function send(message){const input=message.trim();if((!input&&!state.attachments.length)||state.busy)return;openAgent();if(sensitive(input)){addText(input,'user','query');addText(copy[state.lang].noPayment);return;}const kind=/^(да,?\s*добавь|подтверждаю|добавляй|добавь|отмена|нет|жоқ|бас тарту)[.! ]*$/i.test(input)?'action':'query';addText(input||(state.lang==='kz'?'Файл жіберу':'Отправить вложение'),'user',kind);state.busy=true;$('#chat-input').disabled=true;try{let response;if(state.attachments.length){const form=new FormData();form.append('session_id',state.chatId);form.append('message',input);form.append('file',state.attachments[0]);response=await fetch('/api/chat/upload',{method:'POST',body:form});}else{response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:input,session_id:state.chatId})});}const data=await response.json();if(!response.ok)throw new Error(data.detail||'Ошибка API');await applyBackendResponse(data);}catch(error){addText(`Сервер недоступен: ${error.message}`);}finally{state.busy=false;$('#chat-input').disabled=false;state.attachments=[];$('#attachment-list').hidden=true;$('#attachment-list').replaceChildren();$('#file-input').value='';$('#chat-input').value='';$('#chat-input').focus();scrollChat();}}
function attach(files){const valid=/\.(xlsx|xls|docx|doc|pdf|jpe?g|png|webp|csv)$/i;const accepted=[...files].filter(file=>valid.test(file.name)&&file.size<=10*1024*1024);state.attachments=accepted.slice(0,1);const root=$('#attachment-list');root.replaceChildren();root.hidden=!state.attachments.length;state.attachments.forEach(file=>root.append(element('span','',file.name)));if(accepted.length>1)addText('За один запрос можно отправить один файл. Выбран первый.');if(files.length&&!state.attachments.length)addText(state.lang==='kz'?'Excel, Word, PDF, CSV, JPG, PNG немесе WEBP файлын тіркеңіз (10 МБ дейін).':'Прикрепите Excel, Word, PDF, CSV, JPG, PNG или WEBP размером до 10 МБ.');}

async function initializeBackend(){try{const response=await fetch('/api/products');if(response.ok){const data=await response.json();products.splice(0,products.length,...data.products.map(adaptBackendProduct));renderCatalog();}await refreshServerCart();}catch{addText('Backend недоступен. Проверьте, что сервер запущен.');}}
...
window.QuantMedia?.configure({onAsk(product){send(`Покажи товар: ${product.sku}`);},onBuy(product){openAgent();const bubble=addText('Выберите количество товара:');productCard(product,bubble);scrollChat();}});
renderCatalog();renderCart();if(currentChat()){renderConversation();renderHistory();}else newChat();initializeBackend();
