...
function productDetails(product){const names=Object.entries(product.characteristics).map(([key,value])=>`${localKey(key)}: ${value}`).join(' · ');return state.lang==='kz'?`${product.name}\nАртикул: ${product.sku}\nБағасы: ${money(product.price)}\nҚалдық: Алматы — ${product.warehouses.Алматы} дана; Астана — ${product.warehouses.Астана} дана\n${names}`:`${product.name}\nАртикул: ${product.sku}\nЦена: ${money(product.price)}\nОстатки: Алматы — ${product.warehouses.Алматы} шт.; Астана — ${product.warehouses.Астана} шт.\n${names}`;}
